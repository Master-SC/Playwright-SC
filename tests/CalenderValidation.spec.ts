...
test('Calender Validation',async({page})=>{
    ...
    const year:string="2028";
    const month:string="5";
    const day:string="16";

    const datesList:string[] = [month,day,year];
    ...
    const dates:any = page.locator('.react-date-picker__inputGroup input')

    for(let i=0; i<dates.count(); i++){
        const value = await dates.nth(i).inputValue()
        expect(value).toEqual(datesList[i])
    }
