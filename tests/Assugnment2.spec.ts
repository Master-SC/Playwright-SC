...
test.describe("Assignmemt for mock", async () => {
    ...
    test("Banner IS visible when 6 events are returned", async ({ page }) => {
        ...
        await page.getByText('Your sandbox holds up to 9').waitFor({state:'visible'});
        expect(await page.getByText('Your sandbox holds up to 9')).toBeVisible()
        expect(await page.getByText('Your sandbox holds up to 9')).toContainText("9 bookings")
