...
test('Booking Flow', async({page})=>{
    ...
    const seatsBeforeBooking = ((await page.locator("[data-testid='event-card']").filter({hasText:eventName}).locator('div.justify-between div span').textContent()).split(" "))[0];
    await page.locator("[data-testid='event-card']").filter({hasText:eventName}).locator('div.justify-between a').click()
    ...
    await page.locator("[data-testid='event-card']").first().waitFor();
    expect (await page.locator("[data-testid='event-card']").filter({hasText:eventName})).toBeVisible({timeout:500})
    const seatsAfterBooking  = ((await page.locator("[data-testid='event-card']").filter({hasText:eventName}).locator('div.justify-between div span').textContent()).split(" "))[0];
    expect(Number(seatsAfterBooking)).toBe(Number(seatsBeforeBooking)-1)
...
test('Single ticket booking is eligible for refund', async({page})=>{
    ...
    const bookingRef = ((await page.locator('span.bg-indigo-50').first().textContent()).split(''))[0];
    const eventTitle = ((await page.locator('h3.text-base').first().textContent()).split(''))[0];
