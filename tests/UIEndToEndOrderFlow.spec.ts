...
test('Order Place Flow', async({page})=>{
    ...
    const title = await page.locator('div.mt-1 p').textContent()
    await expect(title).toBe('Automation Practice')
    ...
    const text1 = await page.locator('td.em-spacer-1 label.ng-star-inserted').nth(0).textContent();
    const text2 = await page.locator('td.em-spacer-1 label.ng-star-inserted').nth(1).textContent();

    const order1 = text1?.split(' ')[2] ?? '';
    const order2 = text2?.split(' ')[2] ?? '';
